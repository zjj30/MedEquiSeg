...
class ConfigurationManager(object):
    ...
    def UNet_base_num_features(self) -> int:
        if 'UNet_base_num_features' in self.configuration:
    ...
    def n_conv_per_stage_encoder(self) -> List[int]:
        if 'n_conv_per_stage_encoder' in self.configuration:
    ...
    def n_conv_per_stage_decoder(self) -> List[int]:
        if 'n_conv_per_stage_decoder' in self.configuration:
    ...
    def num_pool_per_axis(self) -> List[int]:
        if 'num_pool_per_axis' in self.configuration:
            return self.configuration['num_pool_per_axis']
        strides = self.pool_op_kernel_sizes
        return [sum(stage[axis] > 1 for stage in strides) for axis in range(len(strides[0]))]

    @property
    def pool_op_kernel_sizes(self) -> List[List[int]]:
        if 'pool_op_kernel_sizes' in self.configuration:
    ...
    def conv_kernel_sizes(self) -> List[List[int]]:
        if 'conv_kernel_sizes' in self.configuration:
    ...
    def unet_max_num_features(self) -> int:
        if 'unet_max_num_features' in self.configuration:
